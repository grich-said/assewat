...
class geojson_transformer():
    def __init__(self, dataset):
        self.dataset = dataset
        self.output_dir = "/kaggle/working/ndvi/"
    ...
    def to_raster(self):
        for dict_ in self.dataset:
            ...
            for image_dict in dict_["images"]:
                ...
                if image_id.startswith("COPERNICUS/S2_SR/"):
                    image_id = image_id[len("COPERNICUS/S2_SR/"):]
                ...
                with rasterio.open(output_file, "w", driver="GTiff", height=ndvi.shape[0], width=ndvi.shape[1], count=1,
                    ...
                    dst.write(ndvi, 1)
                dict_["ndvi"].append(
                    {"date": image_dict["date"], "geometry": image_dict["geometry"], "path": output_file,
